...
async function fetchDbHealth(): Promise<DbStatus> {
  ...
  const now = Date.now();
  // throttle to avoid hammering non-existent route
...
function Pill({ label, tone }: { label: string; tone: "emerald" | "rose" | "zinc" }) {
  ...
  return (
    <span className={`px-2 py-[3px] rounded-md text-[11px] font-mono border ${map[tone]}`}>{label}</span>
  );
}

/* ------------------------------- component ------------------------------- */
// --- Shell wrapper (right-side vertical HomeBar) ---
...
export default function HomeBar({
  ...
  // keep latest metMute in a ref to avoid stale closure in effects
  const metMuteRef = useRef(metMute);
  useEffect(() => { metMuteRef.current = metMute; }, [metMute]);

  // sync poller state + ping DB health on activity (no audio here; TimerBar owns beeps)
  useEffect(() => {
      ...
      <div className="px-3 py-2 flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-3">
          <div className="hidden md:flex items-center gap-2 text-[11px]">
            ...
            <code className="px-1 rounded border border-zinc-600/40 bg-zinc-800/60 tabular-nums w-10 text-right">{rem40}</code>
            <span className="text-zinc-400 ml-2">120s</span>
            <code className="px-1 rounded border border-zinc-600/40 bg-zinc-800/60 tabular-nums w-12 text-right">{rem120}</code>
          </div>
