...
/**
 * If you expose a direct (non-HTTP) provider in the server gateway,
 * e.g. export async function getMatricesLatestProvider(coins?: string[]) { ... }
 * this route will delegate to it. Otherwise it falls back to the original logic below.
 */
async function tryGatewayProvider(coins: string[]) {
  try {
    const gw = await import("@/lib/dynamics.gateway.server");
    const fn: undefined | ((coins?: string[]) => Promise<any>) =
      (gw as any)?.getMatricesLatestProvider; // ← implement this in the gateway when ready
    if (typeof fn === "function") {
      const payload = await fn(coins.length ? coins : undefined);
...
export async function GET(req: Request) {
  ...
  // coin universe
  const explicit = coinsFromQuery(url.searchParams.get("coins"));
  ...
  coins = uniqUpper(coins);

  // 1) Try server gateway provider (non-HTTP). If available, delegate and return.
  ...
  // 2) Fallback: original in-route build (unchanged)
  ...
  for (let i = 0; i < n; i++)
    for (let j = 0; j < n; j++) {
      ...
      const pA = lastUSDT[A],
        pB = lastUSDT[B];
      const qA = pctUSDT_percent[A],
        qB = pctUSDT_percent[B];
      const dA = dltUSDT_abs[A],
        dB = dltUSDT_abs[B];
    ...
    if (math?.buildDerived && db?.getPrevValue) {
      ...
      if (prevB) {
        for (let i = 0; i < n; i++)
          for (let j = 0; j < n; j++) {
            if (i === j) continue;
            const cur = benchmark[i][j],
              prv = prevB?.[i]?.[j];
            id_pct[i][j] =
              typeof cur === "number" && typeof prv === "number" && prv !== 0 ? (cur - prv) / prv : null;
          }
      }
  ...
  prevBenchByKey[key] = Array.from({ length: n }, (_, i) =>
    Array.from({ length: n }, (_, j) => (typeof benchmark[i][j] === "number" ? (benchmark[i][j] as number) : NaN))
  );
  prevIdPctByKey[key] = Array.from({ length: n }, (_, i) =>
    Array.from({ length: n }, (_, j) => (typeof id_pct[i][j] === "number" ? (id_pct[i][j] as number) : NaN))
  );
