 ...
 *  Works whether the adapter exports fetchOrderBooksForCoins or fetchOrderBooksForSymbols.
 */
...
export async function GET(req: Request) {
  ...
  try {
    ...
    // keep empty; we’ll still try direct pairs and percent math
  }
  ...
  for (const c of coins) {
    ...
    const tLast = t?.price ?? NaN;      // numeric (adapter-normalized)
    const pctp = t?.pct24h ?? NaN;      // percent number (e.g., 0.93 = 0.93%)

    // prefer OB mid, else ticker price
    ...
    // absolute delta vs USDT (needed for bridged Δ when no direct A/B)
    if (c === "USDT") {
      dltUSDT_abs[c] = 0;
    } else if (lastUSDT[c] != null && Number.isFinite(pctp)) {
      const r = (pctp as number) / 100;
      const open = lastUSDT[c]! / (1 + r);
      dltUSDT_abs[c] = lastUSDT[c]! - open; // absolute delta
    } else {
      dltUSDT_abs[c] = null;
    }
  }
  ...
  const pct24h: Grid = mkGrid(n);   // percent number (UI prints "%")
  const delta: Grid = mkGrid(n);    // absolute
  const id_pct: Grid = mkGrid(n);   // fraction
  const pct_drv: Grid = mkGrid(n);  // fraction
  ...
  for (let i = 0; i < n; i++) for (let j = 0; j < n; j++) {
    ...
    // pct24h (percent units)
    if (qA != null && qB != null) {
      const relPct = ((1 + qA / 100) / (1 + qB / 100) - 1) * 100;
      pct24h[i][j] = relPct;
    }

    // delta (absolute) via first-order expansion
  ...
  for (let i = 0; i < n; i++) for (let j = 0; j < n; j++) {
    ...
    // direct percent (and exact inverse percent)
    ...
    // direct absolute delta (and exact inverse delta if we have prices)
  ...
  try {
    ...
    if (math?.buildDerived && db?.getPrevValue) {
      ...
      // fallback to in-memory snapshots (keeps route stable even without DB)
    ...
    // leave derived as nulls; UI remains stable
  }
  ...
  // send real flags so purple shows up
